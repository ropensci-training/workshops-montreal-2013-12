...
comnames <- sci2comm(splist, simplify=TRUE)
...
comnames_vec <- sapply(comnames, function(x) as.character(x$vernacularName[[1]]), USE.NAMES=FALSE)
...
mynames <- lapply(allnames_df$spname, function(x) name_backbone(name=x, kingdom='plants')$speciesKey)
mynames <- compact(mynames)
occurr_list <- occ_search(taxonKey=mynames, georeferenced=TRUE, limit=50, return='data')
occurr_data <- ldply(occurr_list)
```
